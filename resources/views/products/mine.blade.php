...
<div class="d-flex flex-wrap justify-content-between align-items-center gap-2 mb-3">
    <h4 class="mb-0">👤 منتجاتي</h4>
    <a href="{{ route('products.index') }}" class="btn btn-secondary">⬅ كل المنتجات</a>
</div>

<form method="GET" action="{{ route('products.mine') }}" class="card shadow-sm p-3 mb-3">
    <div class="row g-2 align-items-end">

        <div class="col-md-4">
            <label class="form-label">بحث</label>
            <input type="text" name="search" class="form-control"
                   value="{{ request('search') }}" placeholder="بحث عن منتج...">
        </div>
        ...
        <div class="col-md-12 d-flex justify-content-end">
            <a href="{{ route('products.mine') }}" class="btn btn-outline-secondary btn-sm">تصفير</a>
        </div>
...
<table class="table table-hover align-middle mb-0">
    ...
    <tbody>
        ...
        <tr>
            <td class="fw-bold">{{ $products->firstItem() + $i }}</td>
            ...
            <td>
                @if($p->user)
                    <span class="badge bg-info text-dark">{{ $p->user->name }}</span>
                @else
            ...
            <td>{{ optional($p->updated_at)->format('Y-m-d') }}</td>
        ...
        @empty
        <tr>
            <td colspan="10" class="text-center text-muted py-4">ما عندك منتجات بعد.</td>
        </tr>
        @endforelse
    </tbody>
</table>
