...
<div class="d-flex flex-wrap justify-content-between align-items-center gap-2 mb-3">
    ...
    <a href="{{ route('products.create', ['return' => request()->fullUrl()]) }}" class="btn btn-success">
        ➕ إضافة منتج
    </a>
</div>

<form method="GET" action="{{ route('products.index') }}" class="card shadow-sm p-3 mb-3">
...
<table class="table table-hover align-middle mb-0">
    ...
    <tbody>
        ...
        <tr>
            ...
            <td>{{ optional($p->category)->name ?? 'بدون صنف' }}</td>

            <td>{{ optional($p->created_at)->format('Y-m-d') }}</td>
            <td>{{ optional($p->updated_at)->format('Y-m-d') }}</td>

            <td class="d-flex flex-wrap gap-1">
                <a href="{{ route('products.show', $p) }}?return={{ urlencode(request()->fullUrl()) }}"
                   class="btn btn-outline-info btn-sm">عرض</a>

                <a href="{{ route('products.edit', $p) }}?return={{ urlencode(request()->fullUrl()) }}"
