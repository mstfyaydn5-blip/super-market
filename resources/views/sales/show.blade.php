...
@section('page_title', 'تفاصيل فاتورة البيع')
@section('page_subtitle', 'عرض كامل لتفاصيل فاتورة البيع')
...
<div class="invoice-shell">
    ...
    <div>
        <h4 class="fw-bold mb-1">فاتورة بيع</h4>
        <small class="text-muted">تفاصيل كاملة للطباعة والمراجعة</small>
    </div>

    <div class="d-flex gap-2">
        <a href="{{ route('sales.index') }}" class="btn btn-outline-secondary rounded-3">
            رجوع
        </a>

        @if(($sale->remaining_amount ?? 0) > 0 && $sale->customer_id)
            <a href="{{ route('customer.payments.create', $sale->customer_id) }}" class="btn btn-warning rounded-3">
                تسديد المتبقي
            </a>
        @endif

        <button type="button" onclick="window.print()" class="btn btn-success rounded-3">
        ...
        <div class="invoice-head">
            <div class="d-flex flex-wrap justify-content-between align-items-center gap-3">
                <div>
                    ...
                    <div class="invoice-sub">عرض تفاصيل الفاتورة والمبالغ والمواد المباعة</div>
                </div>
            ...
            <div class="row g-3 mb-4">
                <div class="col-md-4">
                    <div class="info-card">
                        ...
                        <div class="info-value">{{ $sale->customer->name ?? 'نقدي' }}</div>
                    </div>
                </div>
                ...
                <div class="col-md-4">
                    <div class="info-card">
                        ...
                        <div class="info-value">{{ \Carbon\Carbon::parse($saleDate)->format('Y-m-d') }}</div>
                    </div>
                </div>
                    ...
                    <tbody>
                        @forelse($sale->items as $index => $item)
                            <tr>
                                ...
                                <td>{{ $item->quantity }}</td>
                                <td>{{ number_format($item->price ?? $item->sale_price ?? 0, 2) }}</td>
            ...
            <div class="row g-3 mb-4">
                ...
                <div class="col-lg col-md-6">
                    <div class="summary-card bg-total">
                        ...
                        <div class="summary-value">{{ number_format($sale->total, 2) }}</div>
                    </div>
                </div>
                ...
                <div class="col-lg col-md-6">
                    <div class="summary-card bg-paid">
                        ...
                        <div class="summary-value">{{ number_format($sale->paid_amount ?? 0, 2) }}</div>
                    </div>
                </div>
                ...
                <div class="col-lg col-md-6">
                    <div class="summary-card bg-remaining">
                        ...
                        <div class="summary-value">{{ number_format($sale->remaining_amount ?? 0, 2) }}</div>
                    </div>
                </div>
            </div>
